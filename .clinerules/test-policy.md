...
| -------------- | ----------------------------------------------------------------------------------------- | ----- |
| Auth           | `packages/backend/src/controllers/auth/tests/auth.controller.spec.ts`                     | 12    |
...
**Итого (бэкенд):** 170 suites, 1115 тестов (unit 588 + shared 377 + validators 150), включая integration-тесты всех 10 контроллеров, unit-тесты guards/interceptors/decorators и unit-тесты libs/views/config.
...
| 🥈 4      | **Features (фичи)** | `features/auth/`, `features/dashboard-view/`, `features/company/`, `features/partner/`, `features/user/`                                              | ✅ unit (company, dashboard-data, dashboard-templates, hints, ui, user — сессия 46)                                                                                                                                                    |
| 🥈 5      | **Widgets**         | `widgets/auth/`, `widgets/sidebar/`, `widgets/navbar/`, `widgets/footer/`, `widgets/dashboard-view/`, `widgets/dashboard-render/`                     | ✅ smoke (auth/accept-cookie, sidebar, navbar, footer, dashboard-view/panel, dashboard-data/datebar, hints, message-bar, page-loader, version, logo-btn, offers, page-error, demo/goto-demo-btn, ui-configurator, dashboard-templates) |
| 🥉 6      | **Pages**           | Статические страницы: `not-found`, `not-access`, `policy` (smoke) + `root`, `demo`, `company`, `company-profile`, `user-profile`, `dashboard` (smoke) | ✅ smoke                                                                                                                                                                                                                               |
...
**Итого (весь проект):** 170 suites (backend) + 446 suites (frontend) + 6 suites (e2e) = **622 suites**;
1115 тестов (backend) + 3093 тестов (frontend) + 22 теста (e2e) = **4230 тестов**.
