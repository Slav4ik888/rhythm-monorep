...
export const SetLinearGradient: FC<Props> = memo(
  ({ field, selectedItem, sx, defaultValue = '', gradients, onChange }) => {
    ...
    const handleDeg = useCallback(
      (e: MouseEvent, value: number | string) => {
    ...
    const handleGrainentMain = useCallback(
      (value: string) => {
        setMain(value);
    ...
    const handleGrainentState = useCallback(
      (value: string) => {
        setState(value);
    ...
    return (
      <>
        <InputByScheme
          ...
          onSubmit={handleDeg}
        />
        <ColorPicker defaultColor={main} sx={sx} onChange={handleGrainentMain} />
        <ColorPicker defaultColor={state} sx={sx} onChange={handleGrainentState} />
      </>
    );
  },
);
