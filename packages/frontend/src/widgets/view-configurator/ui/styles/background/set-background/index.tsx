...
export const SetBackground: FC<Props> = memo(({ field, selectedItem, onChange }) => {
  const gradients = useMemo(() => splitGradinetRgba(selectedItem?.styles?.[field] as string), [field, selectedItem]);
  ...
  const handleBackground = useCallback(
    (value: string) => onChange(field, value as unknown as string, 'SetBackground'),
    [field, onChange],
  );

  return (
    <RowWrapper>
      <ConfiguratorTextTitle bold title={field} toolTitle='background' />
      <Tooltip title='Настроить gradient'>
        <Checkbox
          size='small'
          checked={checked}
          slotProps={{ input: { 'aria-label': field } }}
