// packages/frontend/src/widgets/view-configurator/ui/styles/alignment/flex-panel/index.tsx
...
export const FlexPanelAlignment: FC<Props> = memo(({ selectedItem, onChange }) => (
  <RowWrapper sx={{ root: { ...f('c-fs-sb'), gap: 1 } }}>
    <Stack direction='row' spacing={1} sx={{ justifyContent: 'space-between', width: '100%' }}>
      <FlexDirection value={selectedItem?.styles?.flexDirection as FlexDirectionType} onChange={onChange} />
      <FlexWrap value={selectedItem?.styles?.flexWrap as FlexWrapType} onChange={onChange} />
    </Stack>

    <Stack spacing={1} sx={{ alignItems: 'flex-end', width: '100%' }}>
      <AlignItems value={selectedItem?.styles?.alignItems as AlignItemsType} onChange={onChange} />
      <JustifyContent value={selectedItem?.styles?.justifyContent as JustifyContentType} onChange={onChange} />
    </Stack>
  </RowWrapper>
));
