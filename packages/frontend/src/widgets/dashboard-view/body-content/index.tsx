// packages/frontend/src/widgets/dashboard-view/body-content/index.tsx
...
import type { ActivatedCopiedType } from 'entities/dashboard-view';
...
export const DashboardBodyContent = memo(() => {
  ...
  const handleSelectViewItem = useCallback(
    (id: ViewItemId) => {
      if (!editMode) return;
      ...
      if (activatedMovementId) {
      ...
      } else if ((activatedCopied as ActivatedCopiedType)?.type === 'copyStyles') {
        if (selectedId === id) return; // Нажали на этот же элемент
        ...
        if (isUnsaved) {
          /** Сохраняем текущий элемент, если он не сохранен */
      ...
      // НЕ меняя selectedId (чтобы не активировать NO_PARENT_ID)
        ...
        if (
          ...
          id !== NO_PARENT_ID && // в корневой элемент
          entities[(activatedCopied as ActivatedCopiedType).id!]?.sheetId !== dashboardSheetId // и другой лист
        )
          return;
