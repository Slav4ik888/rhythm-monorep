...
export const ItemDigitIndicator: FC<Props> = memo(({ item, isTemplate }) => {
  ...
  const [lastValue, prevValue] = isTemplate ? [152350, 121500] : getReversedIndicators(statisticItem?.data, count);
  ...
  const calcedValue: ValueStringAndReduction = useMemo(() => {
    ...
    if (endingDiffType === '% соотношение') {
      value = getFixedFraction(calcGrowthChange(lastValue, prevValue), { fractionDigits, addZero });
