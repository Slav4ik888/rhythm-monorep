...
export const selectModule = createSelector(
  [(state: StateSchema) => state.dashboardData || ({} as SSDD)],
  (state: SSDD) => state,
);
...
export const makeSelectItemByKod = (kod: string | undefined) =>
  createSelector([selectStartEntities], (startEntities: DashboardDataEntities) => startEntities[kod || ''] || '');
