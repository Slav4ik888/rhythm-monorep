...
export const Checkbox: FC<Props> = memo((props) => {
  const { box, toolTitle, label, sx: styles, scheme, disabled, group: G } = props,
    sx = useStyles(styles),
    ...
    <Wrap {...props}>
      <Tooltip title={toolTitle || ''}>
        <FormControlLabel
          ...
          control={<MuiCheckbox disabled={disabled} checked={getValueByScheme(G.group, scheme)} sx={sx.checkbox} />}
        />
      </Tooltip>
    </Wrap>
  );
