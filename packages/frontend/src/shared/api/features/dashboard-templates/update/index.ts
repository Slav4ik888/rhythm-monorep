// packages/frontend/src/shared/api/features/dashboard-templates/update/index.ts

import { createAsyncThunk } from '@reduxjs/toolkit';
import { ThunkConfig, errorHandlers, CustomAxiosError } from 'app/providers/store';
import type { Template, PartialTemplate } from 'entities/dashboard-templates';
...
/** v.2025-06-28 */
export const updateTemplate = createAsyncThunk<UpdateTemplate, UpdateTemplate, ThunkConfig<Errors>>(
  'features/dashboardTemplates/updateTemplate',
  async (updateData, thunkApi) => {
    const { extra, dispatch, rejectWithValue } = thunkApi;

    try {
      const { data } = await extra.api.post<UpdateTemplate>(API_PATHS.templates.update, updateData);
