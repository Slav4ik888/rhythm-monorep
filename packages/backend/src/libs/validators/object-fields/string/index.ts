// packages/backend/src/libs/validators/object-fields/string/index.ts
...
export const validateString = (data: ContainsField, field: string, options: ValidateStringOptions = {}): Validation => {
  const { required, length, min, max } = options,
  ...
  // Check typeof
  if (isHasField(data, field) && isNotStr(value)) {
    return getValidResult({ [field]: ErrorText.INVALID_DATA });
  }
