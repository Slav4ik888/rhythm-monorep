...
describe('TemplatesController — rate limiting (429)', () => {
  let app: NestFastifyApplication;
  ...
  it('GET /api/templates/getBunchesUpdated: превышение лимита (2) → 429', async () => {
    getBunchesUpdatedModelMock.mockResolvedValue({ b1: 123 });

    const responses = [];
    for (let i = 0; i < 3; i += 1) {
      // eslint-disable-next-line no-await-in-loop
      responses.push(await get(app, '/api/templates/getBunchesUpdated'));
    }
