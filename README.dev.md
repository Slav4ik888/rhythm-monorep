...
| `e2e/pwa/offline.spec.ts`      | PWA на production-сборке: офлайн-рендер главной, дашборда и гостевых страниц (вход/регистрация/политика/404) из SW-кэша + проверка, что запросы обслуживаются из кэша (`expectServedFromCache`) — 6 тестов, отдельный конфиг |
